# 图片管理指南

## 图片命名规范

所有产品图片按照统一的命名规范，便于管理和识别：

### 格式
```
产品编号_图片序号.jpg
```

### 示例
- `801_1.jpg` - 801#涤天丝（曲天丝）主图
- `801_2.jpg` - 801#涤天丝（曲天丝）第2张图
- `802_1.jpg` - 802#天丝仙女条主图
- `901_1.jpg` - 901#2060洗水竹节主图

## 图片存储方案

### 方案：本地存储（推荐用于开发测试）

**位置：** `public/images/products/`

**文件结构：**
```
public/
└── images/
    ├── products/
    │   ├── 801_1.jpg
    │   ├── 801_2.jpg
    │   ├── 802_1.jpg
    │   └── ...
    └── banners/
        ├── banner_1.jpg
        ├── banner_2.jpg
        └── banner_3.jpg
```

**产品数据中的配置：**
```json
{
  "id": "prod-801",
  "productNo": "801#",
  "name": "涤天丝（曲天丝）",
  "images": [
    "/images/products/801_1.jpg",
    "/images/products/801_2.jpg"
  ]
}
```

### 替换图片演示

✅ 准备图片 → 按照命名规范命名：826_1.jpg, 826_2.jpg, 826_3.jpg
✅ 放入目录 → 复制到 public/images/products/ 目录
✅ 运行同步 → 执行 node scripts/manage-images.js sync
✅ 自动关联 → 脚本自动更新 data/products.json
现在 #826 产品在网站上就能正常显示您上传的这3张图片了！刷新页面即可看到效果。
💡 提示：
图片文件较大（4-6MB），建议使用图片压缩工具（如 TinyPNG）优化后再上传，可以：
减少加载时间
节省带宽
提升用户体验
这就是整个图片管理的流程，非常简单！以后添加任何产品的图片都按这个方法操作即可。



## 图片规格要求

### 产品图片
- **格式：** JPG/JPEG 或 WebP
- **尺寸：** 
  - 最小：800x800px
  - 推荐：1200x1200px
  - 最大：2000x2000px
- **宽高比：** 1:1（正方形）
- **文件大小：** 不超过 500KB
- **背景：** 建议纯白色或浅色背景

### 轮播图
- **格式：** JPG/JPEG 或 WebP
- **尺寸：** 1600x600px
- **宽高比：** 8:3
- **文件大小：** 不超过 800KB

## 图片更新流程

### 方法一：直接修改 JSON 文件（推荐）

1. 上传新图片到图片存储位置
2. 打开 `data/products.json` 文件
3. 找到对应产品的 `images` 字段
4. 更新图片链接
5. 保存文件
6. 重启开发服务器或重新部署

**示例：**
```json
{
  "id": "prod-801",
  "productNo": "801#",
  "name": "涤天丝（曲天丝）",
  "images": [
    "/images/products/801_1.jpg",  ← 修改这里
    "/images/products/801_2.jpg"
  ]
}
```

### 方法二：使用图片管理脚本

创建 `scripts/update-product-images.js` 脚本：

```javascript
const fs = require('fs');

// 批量更新图片
function updateProductImages() {
  const products = JSON.parse(fs.readFileSync('data/products.json', 'utf8'));
  
  products.forEach(product => {
    // 提取产品编号（如：801# -> 801）
    const productNum = product.productNo.replace('#', '');
    
    // 自动设置图片路径
    product.images = [
      `/images/products/${productNum}_1.jpg`,
      `/images/products/${productNum}_2.jpg`
    ];
  });
  
  fs.writeFileSync('data/products.json', JSON.stringify(products, null, 2));
  console.log('图片链接更新完成！');
}

updateProductImages();
```

**使用方法：**
```bash
node scripts/update-product-images.js
```

## 批量图片上传

### 使用阿里云OSS命令行工具

```bash
# 安装ossutil
# Windows: 下载 ossutil64.exe
# Mac: brew install ossutil

# 配置
ossutil config

# 批量上传
ossutil cp -r ./public/images/products/ oss://tx-fabric-bucket/products/
```

### 使用图床批量上传工具

推荐工具：
- **PicGo** - 跨平台图床上传工具
- **uPic** - Mac专用
- **PicList** - PicGo的增强版

## 图片优化建议

### 1. 自动压缩
```bash
# 安装压缩工具
npm install -g imagemin-cli

# 压缩图片
imagemin public/images/products/*.jpg --out-dir=public/images/products/optimized
```

### 2. 使用 Next.js Image 组件
项目已经使用 `next/image` 组件，自动优化：
- 自动调整尺寸
- 懒加载
- WebP格式转换

### 3. 配置图片域名白名单
在 `next.config.js` 中配置：
```javascript
module.exports = {
  images: {
    domains: [
      'images.unsplash.com',
      'your-bucket.oss-cn-shenzhen.aliyuncs.com',
      'i.loli.net'
    ],
  },
}
```

## 快速参考

### 产品编号对照表

| 系列 | 编号范围 | 数量 |
|------|---------|------|
| 天丝系列 | 801# - 832# | 28个 |
| 绣花底布 | 601# - 606# | 6个 |
| 棉布系列 | 901# - 914# | 13个 |

### 常见问题

**Q: 图片不显示怎么办？**
A: 检查以下几点：
1. 图片路径是否正确
2. 图片文件是否存在
3. 图片域名是否在白名单中
4. 开发服务器是否重启

**Q: 如何更换轮播图？**
A: 修改 `data/banners.json` 文件中的 `image` 字段

**Q: 如何添加多张产品图？**
A: 在 `images` 数组中添加更多图片URL：
```json
"images": [
  "/images/products/801_1.jpg",
  "/images/products/801_2.jpg",
  "/images/products/801_3.jpg"  ← 新增
]
```

## 自动化方案（进阶）

### 创建图片管理脚本
保存为 `scripts/manage-images.js`：

```javascript
const fs = require('fs');
const path = require('path');

// 扫描本地图片并自动更新JSON
function syncProductImages() {
  const productsData = JSON.parse(fs.readFileSync('data/products.json', 'utf8'));
  const imageDir = 'public/images/products';
  
  if (!fs.existsSync(imageDir)) {
    fs.mkdirSync(imageDir, { recursive: true });
  }
  
  productsData.forEach(product => {
    const productNum = product.productNo.replace('#', '');
    const images = [];
    
    // 查找所有该产品的图片
    for (let i = 1; i <= 5; i++) {
      const imagePath = path.join(imageDir, `${productNum}_${i}.jpg`);
      if (fs.existsSync(imagePath)) {
        images.push(`/images/products/${productNum}_${i}.jpg`);
      }
    }
    
    if (images.length > 0) {
      product.images = images;
    }
  });
  
  fs.writeFileSync('data/products.json', JSON.stringify(productsData, null, 2));
  console.log(`✓ 已同步 ${productsData.length} 个产品的图片`);
}

syncProductImages();
```

**使用：**
```bash
# 1. 将图片按命名规范放入 public/images/products/
# 2. 运行脚本自动更新JSON
node scripts/manage-images.js
```

---

**维护者提示：** 
- 定期清理未使用的图片
- 保持命名规范一致
- 做好图片备份
- 建议使用CDN加速图片加载

